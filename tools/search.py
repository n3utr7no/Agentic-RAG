...
def search_node(query: str):
    ...
    queries_obj = light_model.with_structured_output(Queries, method='json_mode').invoke(SEARCH_PROMPT.format(query=query) + '\nRespond in JSON.')
...
def search(queries: Queries, max_results=5):
    ...
    print('----Searching----')
    all_results = [res['content'] for query in queries.queries for res in client.search(query, max_results=max_results)['results']]
    return all_results
